...
const StructureSchema = z.object({
  ...
  blocks: z.array(BlockSchema).refine(
    (blocks) => {
      const requiredBlocks = ['TableBlock', 'TotalBlock', 'ClientInfoBlock'];
      const blockTypes = blocks.map((block) => block.type);
      return requiredBlocks.every((type) => blockTypes.includes(type));
    },
    { message: 'Missing required blocks (TableBlock, TotalBlock, ClientInfoBlock)' }
  ),
});
...
const TemplateSchema = z.object({
  ...
  type: z.enum(['invoice', 'quote', 'receipt'], {
    errorMap: () => ({ message: 'Type must be invoice, quote, or receipt' }),
  }),
