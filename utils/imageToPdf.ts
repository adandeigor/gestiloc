...
export async function convertImageToPDF(file: File): Promise<{
    data: { file: File; path: string } | null;
    error: string | null;
}> {
    try {
        if (!file) {
        ...
        if (!validTypes.includes(file.type)) {
            return {
                data: null,
                error: 'Le fichier doit être au format JPG, JPEG ou PNG',
            };
        }
        ...
        try {
            ...
            console.error(
                'Erreur lors de la lecture des métadonnées avec sharp :',
                sharpError
            );
            return {
                data: null,
        ...
        const res = await fetch(
            'https://gwsosglvvobbfayijeri.supabase.co/storage/v1/object/public/gestionnaire/fonts/open-sans.ttf'
        );
        ...
        try {
            ...
            console.error(
                "Erreur lors de l'ajout de l'image au PDF :",
                imageError
            );
            return {
                data: null,
                error: "Erreur lors de l'ajout de l'image au PDF",
            };
        }
        ...
        return {
            data: null,
            error: `Erreur lors de la conversion en PDF : ${(error as Error).message}`,
        };
    }
}
