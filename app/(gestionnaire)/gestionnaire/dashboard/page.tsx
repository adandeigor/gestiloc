...
export default function Dashboard() {
  ...
  if (!userStats) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <span className="text-red-500">Impossible de charger les statistiques.</span>
      </div>
    );
  }
  ...
  const handleDeleteAudits = async () => {
    ...
    try {
      const response = await fetch(`/api/user/${userStats.gestionnaire?.id}/auditlog`, {
        method: 'DELETE',
        headers: {
      ...
      <div className="max-w-7xl mx-auto py-6 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard
            ...
            value={unitsAvailable > 0 ? unitsAvailable : 'Aucune unité disponible'}
            icon={CheckCircle}
        ...
        <div className="mt-8">
          <Card>
            <CardContent className="p-4">
                    ...
                    >
                      ...
                      {showCheckboxes && (
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => handleCheckboxChange(log.id, !!checked)}
                          className="mr-2"
                        />
                      )}
                    </motion.li>
              ...
              {!auditLogs.length && (
                <div className="text-center text-gray-400 py-4">Aucune action récente.</div>
              )}
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="destructive">
                        {selectedAuditIds.length === 1 ? 'Supprimer' : 'Tout supprimer'}
                      </Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader>
