...
const ProprieteSchema = z.object({
    ...
    ville: z
        .string()
        .min(1, 'La ville de localisation de la propriété est requise'),
...
export function ProprieteDialog({
    open,
    onClose,
    userId,
    propriete,
}: ProprieteDialogProps) {
    const {
        register,
        handleSubmit,
        setValue,
    ...
    const handleGetLocation = (): void => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error: GeolocationPositionError) => {
                toast.error(
                    'Impossible de récupérer la localisation : ' + error.message
                );
                setLocationLoading(false);
            }
        );
        ...
        try {
            if (propriete) {
                ...
                await httpClient.put(
                    `/api/user/${userId}/propriete/${propriete.id}`,
                    data
                );
            ...
            >
                ...
                >
                    ...
                    <div>
                        ...
                        {errors.localisation?.latitude && (
                            <span className="text-red-500 text-xs">
                                {errors.localisation.latitude.message}
                            </span>
                        )}
                        {errors.localisation?.longitude && (
                            <span className="text-red-500 text-xs">
                                {errors.localisation.longitude.message}
                            </span>
                        )}
                    </div>
