    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                const proprietes =
                    (stats as { proprietes?: unknown[] }).proprietes || [];
                interface Locataire {
                    id: string;
                    [key: string]: unknown;
                }
                ...
                const labels: string[] = (proprietes as Propriete[]).map(
                    p => p.nom
                );
                const data: number[] = (proprietes as Propriete[]).map(
                    (p: Propriete) => {
                        // Compte le nombre de locataires uniques par propriété
                        const locataires: Set<string> = new Set();
                        (p.unitesLocatives || []).forEach(
                            (u: UniteLocative) => {
                                (u.locataires || []).forEach((l: Locataire) =>
                                    locataires.add(l.id)
                                );
                            }
                        );
