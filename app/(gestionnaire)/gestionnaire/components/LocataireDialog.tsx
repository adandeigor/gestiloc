...
const LocataireSchema = z.object({
  ...
  carte_identite: z.union([z.string(), z.instanceof(File)]).refine(val => typeof val === 'string' ? val.length > 0 : true, "La carte d'identité du locataire est requise"),
  photo_identite: z.union([z.string(), z.instanceof(File)]).optional().nullable(),
  ...
  message: "L'unité locative est requise si une propriété est sélectionnée",
...
export function LocataireDialog({ open, onClose, userId, proprietes, locataire }: LocataireDialogProps) {
  const { register, handleSubmit, setValue, watch, formState: { errors }, reset } = useForm<LocataireFormType>({
    ...
    defaultValues: {
      ...
      proprieteId: '',
      uniteLocativeId: locataire?.uniteLocativeId ? String(locataire.uniteLocativeId) : '',
    },
  });
  ...
  useEffect(() => {
    if (locataire && proprietes.length > 0) {
      ...
      if (prop) {
        ...
        console.log('Pre-selected uniteLocativeId:', String(locataire.uniteLocativeId));
      }
    } else {
      setUnitOptions([]);
  ...
  useEffect(() => {
    if (selectedProprieteId) {
      const prop = proprietes.find(p => String(p.id) === String(selectedProprieteId));
      setUnitOptions(prop?.unitesLocatives || []);
      if (locataire && prop?.unitesLocatives.some(u => u.id === locataire.uniteLocativeId)) {
        setValue('uniteLocativeId', String(locataire.uniteLocativeId));
      } else if (!locataire || !prop?.unitesLocatives.some(u => u.id === locataire.uniteLocativeId)) {
        setValue('uniteLocativeId', '');
      }
    } else {
      setUnitOptions([]);
  ...
  const onSubmit = async (data: LocataireFormType) => {
    if (locataire && data.uniteLocativeId && data.uniteLocativeId !== String(locataire.uniteLocativeId)) {
  ...
  const submitLocataire = async (data: LocataireFormType) => {
    ...
    try {
      ...
      if (data.photo_identite instanceof File) {
        ...
        photoUrl = await uploadToSupabase(data.photo_identite, 'profil_picture', String(userId));
        setFileUploading(false);
      }
      if (data.carte_identite instanceof File) {
        setFileUploading(true);
        carteUrl = await uploadToSupabase(data.carte_identite, 'carte_identite', String(userId));
        setFileUploading(false);
      }

      const prop = proprietes.find(p => String(p.id) === String(data.proprieteId));
      const unite = data.uniteLocativeId
        ? prop?.unitesLocatives.find(u => String(u.id) === String(data.uniteLocativeId))
        : locataire
          ? prop?.unitesLocatives.find(u => u.id === locataire.uniteLocativeId)
          : null;
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className="max-w-lg w-full p-2 sm:p-6 rounded-lg overflow-y-auto max-h-[90dvh] sm:max-h-[80vh]" style={{ overscrollBehavior: 'contain' }}>
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg md:text-xl font-semibold text-gray-900">
              {locataire ? 'Modifier un locataire' : 'Ajouter un locataire'}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 sm:space-y-4 text-xs sm:text-sm">
            <div className="flex flex-col md:flex-row gap-2">
              <div className="flex-1">
                ...
                <Input {...register('nom')} placeholder="Nom" disabled={loading} className="text-xs sm:text-sm px-2 py-1 sm:py-2" />
              ...
              <div className="flex-1">
                ...
                <Input {...register('prenom')} placeholder="Prénom" disabled={loading} className="text-xs sm:text-sm px-2 py-1 sm:py-2" />
                {errors.prenom && <span className="text-red-500 text-xs">{errors.prenom.message}</span>}
              </div>
            </div>
            <div>
              <label className="block text-xs sm:text-sm font-medium mb-1">Email</label>
              <Input {...register('email')} placeholder="Email" disabled={loading} className="text-xs sm:text-sm px-2 py-1 sm:py-2" />
              {errors.email && <span className="text-red-500 text-xs">{errors.email.message}</span>}
            </div>
            <div>
              <label className="block text-xs sm:text-sm font-medium mb-1">Téléphone</label>
              <Input {...register('telephone')} placeholder="Téléphone" disabled={loading} className="text-xs sm:text-sm px-2 py-1 sm:py-2" />
            ...
            <div>
              ...
              <Input
                ...
                onChange={e => {
                  ...
                  setValue('carte_identite', file || locataire?.carte_identite || '');
                }}
                disabled={loading || fileUploading}
                className="text-xs sm:text-sm px-2 py-1 sm:py-2"
              />
              {locataire?.carte_identite && (
                <p className="text-xs text-gray-500 mt-1">Fichier actuel : {locataire.carte_identite.split('/').pop()}</p>
              )}
              {fileUploading && <span className="text-blue-500 text-xs">Téléversement en cours...</span>}
            ...
            <div>
              ...
              <Input
                ...
                onChange={e => {
                  const file = e.target.files?.[0];
                  setValue('photo_identite', file || locataire?.photo_identite || '');
                }}
                disabled={loading || fileUploading}
                className="text-xs sm:text-sm px-2 py-1 sm:py-2"
              />
              {locataire?.photo_identite && (
                <p className="text-xs text-gray-500 mt-1">Fichier actuel : {locataire.photo_identite.split('/').pop()}</p>
              )}
              {fileUploading && <span className="text-blue-500 text-xs">Téléversement en cours...</span>}
              {errors.photo_identite && <span className="text-red-500 text-xs">{errors.photo_identite.message}</span>}
            </div>
            <div>
              <label className="block text-xs sm:text-sm font-medium mb-1">Propriété</label>
              <Select value={selectedProprieteId || undefined} onValueChange={v => setValue('proprieteId', v)} disabled={loading}>
                <SelectTrigger className="text-xs sm:text-sm px-2 py-1 sm:py-2">
                  <SelectValue placeholder="Sélectionner une propriété" />
                </SelectTrigger>
                <SelectContent>
                  {proprietes.length === 0 && <SelectItem value="none" disabled>Aucune propriété disponible</SelectItem>}
                  {proprietes.map((p) => (
                    <SelectItem key={p.id} value={String(p.id)}>{p.nom}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {errors.proprieteId && <span className="text-red-500 text-xs">{errors.proprieteId.message}</span>}
            </div>
            <div>
              <label className="block text-xs sm:text-sm font-medium mb-1">Unité locative {locataire ? '(optionnel)' : ''}</label>
              <Select
                value={selectedUniteLocativeId}
                onValueChange={v => setValue('uniteLocativeId', v)}
              ...
              >
                ...
                <SelectContent>
                  {selectedProprieteId && unitOptions.length === 0 && (
                    <SelectItem value="none" disabled>Aucune unité locative</SelectItem>
                  )}
                  {unitOptions.map((u) => (
              ...
              {errors.uniteLocativeId && <span className="text-red-500 text-xs">{errors.uniteLocativeId.message}</span>}
            </div>
            <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-4 mt-2">
              <Button type="submit" disabled={loading || (!!selectedProprieteId && unitOptions.length === 0 && !locataire) || fileUploading} className="w-full sm:w-auto text-xs sm:text-sm px-2 py-1 sm:py-2">
                {(loading || fileUploading) ? 'Enregistrement...' : locataire ? 'Mettre à jour' : 'Enregistrer'}
              </Button>
              <Button type="button" variant="ghost" onClick={onClose} disabled={loading} className="w-full sm:w-auto text-xs sm:text-sm px-2 py-1 sm:py-2">
                Annuler
              </Button>
            </DialogFooter>
          </form>
